...
<html lang="en">
  ...
  <body class="nav-md">
      ...
      <div class="main_container">
        ...
        <div class="right_col" role="main">
          ...
          <div class="x_content">
            ...
            <form id="add-user-frm" class="form-horizontal form-label-left" method="post" action="<?=base_url('Quotes/do_edit_quote')?>" data-toggle="validator" role="form">
              ...
              <input type="hidden" name="quote-id" value="<?=$quote_id?>">
                    ...
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <div class="col-md-8 col-sm-12 col-xs-12" style="margin-top: 7px;">
                          <?=$quote['quote_no'];?>
                          <input type="hidden" name="quote_no" id="quote_no" value="<?=$quote['quote_no'];?>">
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <div class="col-md-8 col-sm-12 col-xs-12" style="margin-top: 7px;"><?=date('d-M-Y', strtotime($quote["quote_date"]));?></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-md-12">
                  <table class="table table-bordered" id="tblRequestedItems">
                    ...
                    <tbody>
                      <?
                      if( !empty( $quote["quote_details"] ) ){
                        $counter = 1;
                        $items = json_decode( $quote["quote_details"] );
                        foreach ($items as $key => $value) {?>
                          <tr id="<?=$counter?>">
                            ...
                            <td>
                              <a href="#" onClick="delete_quote_item(<?=$counter?>)" title="Delete"><i class="fas fa-trash-alt"></i></a>
                            </td>
                          </tr>
                        <?
                        $counter++;
                    ...
                    <tr>
                      <td colspan="2"></td>
                      <td>Total</td>
                      <td><input type="text" name="total_cost" id="total_cost" class="form-control" readonly="readonly" value="<?= number_format($quote['quote_total_amount'], 2)?>" /></td>
                      <td></td>
                    </tr>
                  </table>
                </div>
              ...
              <div class="row">
                <div class="dol-md-4">
                  <button type="submit" class="btn btn-lg btn-success" style="margin-top: 23px;">Update Quote</button>
                </div>                
              </div>
    ...
    <script type="text/javascript">
      ...
      var quote_total_cost = <?= $quote['quote_total_amount'];?>;
    ...
    function addAnotherRowForRequisitionItem(qty, desc, price, extn){
      ...
      row = row + '<a href="#" onClick="delete_quote_item(' + req_item_counter + ')" title="Delete"><i class="fas fa-trash-alt"></i></a>';
      row = row + '</td>';      
      row = row + '</tr>';

      $("#tblRequestedItems").append(row);

      quote_total_cost += parseFloat(extn);
      console.log(quote_total_cost);
      $("#total_cost").val(quote_total_cost);
    ...
    function delete_quote_item( item_counter ){
      ...
      if( do_delete_row == true ){
        ...
        console.log( extn );
        quote_total_cost = quote_total_cost - extn;
        console.log( quote_total_cost );
